...
def generate_card():
    try:
        ...
        # Step 4: Create feathered mask and blend
        bbox = template_faces[0].bbox.astype(int)
        mask = create_feathered_mask(bbox, template_img.shape)
        blended_img = blend_faces(template_img, swapped_img, mask)

        # Optional: Match skin tone
        #blended_img = match_histograms(blended_img, template_img, multichannel=True)

        blended_img = match_histograms(blended_img, template_img, channel_axis=-1)

        # Step 5: Enhance with GFPGAN
